...
def search_car(finder: tuple):
    ...
    for key, value in car_data.items():
      if result_amount < 5:
        if car_year<=value[1] and car_engine<=value[2] and car_price>=value[4]:
          total_result[key] = value

          result_amount = result_amount+1

    sorted_result = sorted(total_result.items(), key=lambda item: item[1][4])
    for i in sorted_result:
      print(i)
    return

search_car(search_criteria)
